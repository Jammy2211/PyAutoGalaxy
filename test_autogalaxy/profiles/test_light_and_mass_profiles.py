...
def test__gaussian():
    ...
    assert lp.image_2d_from(grid=grid) == pytest.approx(lmp.image_2d_from(grid=grid).array, 1.0e-4)
    assert mp.convergence_2d_from(grid=grid) == pytest.approx(lmp.convergence_2d_from(grid=grid).array, 1.0e-4)
    #    assert (mp.potential_2d_from(grid=grid) == lmp.potential_2d_from(grid=grid)).all()
    assert mp.deflections_yx_2d_from(grid=grid) == pytest.approx(lmp.deflections_yx_2d_from(grid=grid).array, 1.0e-4)
