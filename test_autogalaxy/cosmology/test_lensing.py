...
def test__arcsec_to_kpc_conversion(Planck18):
    arcsec_per_kpc = Planck18.arcsec_per_kpc_from(redshift=0.1)

    assert arcsec_per_kpc == pytest.approx(0.525060, 1e-5)

    kpc_per_arcsec = Planck18.kpc_per_arcsec_from(redshift=0.1)

    assert kpc_per_arcsec == pytest.approx(1.904544, 1e-5)

    arcsec_per_kpc = Planck18.arcsec_per_kpc_from(redshift=1.0)

    assert arcsec_per_kpc == pytest.approx(0.1214785, 1e-5)

    kpc_per_arcsec = Planck18.kpc_per_arcsec_from(redshift=1.0)

    assert kpc_per_arcsec == pytest.approx(8.231907, 1e-5)


def test__angular_diameter_distances(Planck18):
    angular_diameter_distance_to_earth_kpc = (
        Planck18.angular_diameter_distance_to_earth_in_kpc_from(redshift=0.1)
    )

    assert angular_diameter_distance_to_earth_kpc == pytest.approx(392840, 1e-5)

    angular_diameter_distance_between_redshifts_kpc = (
        Planck18.angular_diameter_distance_between_redshifts_in_kpc_from(
...
def test__cosmic_average_densities_solar_mass_per_kpc3(Planck18):
    cosmic_average_density = Planck18.cosmic_average_density_from(redshift=0.6)

    assert cosmic_average_density == pytest.approx(81280.09116133313, 1.0e-4)

    cosmic_average_density = Planck18.cosmic_average_density_solar_mass_per_kpc3_from(
        redshift=0.6
    )
...
def test__critical_surface_mass_densities(Planck18):
    ...
    critical_surface_density = (
        Planck18.critical_surface_density_between_redshifts_solar_mass_per_kpc2_from(
            redshift_0=0.1, redshift_1=1.0
        )
    )
...
def test__velocity_dispersion_from(Planck18):
    velocity_dispersion = Planck18.velocity_dispersion_from(
