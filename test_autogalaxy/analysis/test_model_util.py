...
def test__hyper_galaxy_list_from_model():

    galaxies = af.Collection(galaxy=af.Model(ag.Galaxy, redshift=0.5))

    hyper_galaxy_model_list = ag.util.model.hyper_galaxy_model_list_from(
        model=af.Collection(galaxies=galaxies)
    )

    assert hyper_galaxy_model_list == []

    galaxies = af.Collection(
        galaxy=ag.Galaxy(
            redshift=0.5,
            hyper_galaxy=ag.HyperGalaxy(
                contribution_factor=0.0, noise_factor=0.0, noise_power=0.0
            ),
        )
    )
    ...
    assert isinstance(hyper_galaxy_model_list[0], ag.HyperGalaxy)

    galaxies = af.Collection(
        galaxy=af.Model(ag.Galaxy, redshift=0.5, hyper_galaxy=af.Model(ag.HyperGalaxy))
    )
    ...
    assert hyper_galaxy_model_list[0].cls is ag.HyperGalaxy


def test__has_hyper_galaxy_from_model():

    galaxies = af.Collection(galaxy=af.Model(ag.Galaxy, redshift=0.5))

    has_hyper_galaxy = ag.util.model.has_hyper_galaxy_from_model(
        model=af.Collection(galaxies=galaxies)
    )

    assert has_hyper_galaxy is False
    ...
    assert has_hyper_galaxy is True
...
def test__has_hyper_galaxy_model_from_model():
    ...
    assert has_hyper_galaxy_model is False
    ...
    assert has_hyper_galaxy_model is True


def test__has_hyper_galaxy_instance_from_model():
    ...
    assert has_hyper_galaxy_instance is False
    ...
    assert has_hyper_galaxy_instance is True
