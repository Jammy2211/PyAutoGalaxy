...
def test__mat_obj__all_load_from_config_correctly():
    ...
    assert tangential_critical_curves_plot.config_dict["linewidth"] == 4

    tangential_caustics_plot = aplt.TangentialCausticsPlot()

    assert tangential_caustics_plot.config_dict["linewidth"] == 5

    radial_critical_curves_plot = aplt.RadialCriticalCurvesPlot()

    assert radial_critical_curves_plot.config_dict["linewidth"] == 4

    radial_caustics_plot = aplt.RadialCausticsPlot()

    assert radial_caustics_plot.config_dict["linewidth"] == 5
