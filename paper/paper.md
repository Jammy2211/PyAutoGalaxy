...
![Hubble Space Telescope imaging of a spiral galaxy (left column), a parametric fit to its smooth bulge and disk components (middle column) and  anon-parametric fit to its assymetric and irrergular structures like its spiral arms (right column) using `PyAutoGalaxy`.\label{figure:example}](observed.pdf)
