...
def make_image():
    ...
    noise_map = am.NoiseMap(array=2.0 * np.ones((3, 3)), pixel_scales=1.0)
    psf = aa.kernel(array_1d=3.0 * np.ones((3, 3)), pixel_scales=1.0)

    return am.ImagingData(image=image, pixel_scales=1.0, noise_map=noise_map, psf=psf)
...
def make_galaxy_light():
    return am.Galaxy(
        redshift=0.5, light=am.light_profiles.EllipticalSersic(intensity=1.0)
    )
...
def make_galaxy_mass():
    return am.Galaxy(
        redshift=0.5, mass=am.mass_profiles.SphericalIsothermal(einstein_radius=1.0)
    )
...
def make_rectangular_pixelization():
    return am.pix.Rectangular(shape_2d=(25, 25))
...
def test__image_and_rectangular_mapper_is_output(
    ...
    am.mapper_plotters.plot_image_and_mapper(
        imaging=image,
...
def test__rectangular_mapper_is_output(
    ...
    am.mapper_plotters.plot_mapper(
        mapper=rectangular_mapper,
        should_plot_centres=True,
        should_plot_grid=True,
        image_pixels=[[0, 1, 2], [3]],
