...
class TestGeometryProfile(object):
    def test__constructor_and_units(self):
        profile = am.geometry_profiles.GeometryProfile(centre=(1.0, 2.0))

        assert profile.centre == (1.0, 2.0)
        assert isinstance(profile.centre[0], am.Length)
        assert isinstance(profile.centre[1], am.Length)
...
class TestEllipticalProfile(object):
    class TestConstuctorUnits(object):
        def test__constructor_and_units(self):
            profile = am.geometry_profiles.EllipticalProfile(
    ...
    class TestAnglesFromXAxis(object):
        def test__profile_angle_phi_is_0__cosine_and_sin_of_phi_is_1_and_0(self):
            elliptical_profile = am.geometry_profiles.EllipticalProfile(
    ...
    class TestCoordinateMovements(object):
        def test__grid_and_centres_of_two_lenses_are_equivalent__grid_are_equivalent(
            ...
            elliptical_profile1 = am.geometry_profiles.EllipticalProfile(
                axis_ratio=1.0, phi=0.0, centre=(0, 0)
            )
            grid1 = elliptical_profile1.transform_grid_to_reference_frame(grid=grid_11)

            elliptical_profile2 = am.geometry_profiles.EllipticalProfile(
...
class TestSphericalProfile(object):
    class TestConstuctorUnits(object):
        def test__constructor_and_unit_conversions(self):
            profile = am.geometry_profiles.SphericalProfile(centre=(1.0, 2.0))
    ...
    class TestCoordinatesMovement(object):
        def test__profile_cenre_y_0_x_0__grid_y_1_x_1__no_coordinate_movement_so_y_1_x_1(
            ...
            spherical_profile = am.geometry_profiles.SphericalProfile(centre=(0.0, 0.0))
        ...
        def test__grid_and_centres_of_two_lenses_are_equivalent__grid_are_equivalent(
            ...
            spherical_profile1 = am.geometry_profiles.SphericalProfile(centre=(0, 0))

            grid1 = spherical_profile1.transform_grid_to_reference_frame(grid=grid_11)

            spherical_profile2 = am.geometry_profiles.SphericalProfile(centre=(-1, -1))
        ...
        def test__grid_are_again_the_same_after_centre_shift__grid_equivalent(self):
            spherical_profile1 = am.geometry_profiles.SphericalProfile(centre=(1, 1))
            grid1 = spherical_profile1.transform_grid_to_reference_frame(grid=grid_11)
...
class MockGridRadialMinimum(object):
    ...
    @am.geometry_profiles.move_grid_to_radial_minimum
